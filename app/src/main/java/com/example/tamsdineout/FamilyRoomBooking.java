...
public class FamilyRoomBooking extends AppCompatActivity {
    private ImageView table1,table2,table3,table4,table5,table6,table7,table8;
    private TextView tbl1,tbl2,tbl3,tbl4,tbl5,tbl6,tbl7,tbl8;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_family_room_booking);
    ...
    public void getBookings(){
        ...
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Family Room").child(slotE).child(date);
        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                long count=dataSnapshot.getChildrenCount();
                Toast.makeText(FamilyRoomBooking.this, ""+ count, Toast.LENGTH_LONG).show();
                for(int i=1;i < 17;i++){
                    String ret="book"+i;
                    Toast.makeText(FamilyRoomBooking.this, ""+ret, Toast.LENGTH_LONG).show();
                    if(dataSnapshot.child(ret).exists()){
                        reflectBooking(i);
                        Toast.makeText(FamilyRoomBooking.this, ""+ i, Toast.LENGTH_LONG).show();
                    }
                }
            }
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(FamilyRoomBooking.this, "Error Occurred", Toast.LENGTH_LONG).show();
            }
        });
    }
    private void bookTable(int tableNo){
        ...
        DatabaseReference documentReference = firebaseDatabase.getReference("Family Room").child(slotE);


        documentReference.child(date).child(table).setValue(userI).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Toast.makeText(FamilyRoomBooking.this,"Table Booked",Toast.LENGTH_LONG).show();
                userBooking(userI);
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(FamilyRoomBooking.this,"Table Not Booked",Toast.LENGTH_LONG).show();
            }
        });
        ...
        DatabaseReference documentReference2 = firebaseDatabase2.getReference("Family Room").child(slotE1);
    ...
    public void userBooking(String num){
        ...
        userBookedTable bookedTable=new userBookedTable("FamilyRoom",date,timeFormat,num,"UnPaid");

        mFireStore.collection("Bookings").document(userId).set(bookedTable).addOnCompleteListener(FamilyRoomBooking.this, new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(FamilyRoomBooking.this, "Loading Please Wait", Toast.LENGTH_LONG).show();
                    Handler handler = new Handler();
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            Intent intent = new Intent(FamilyRoomBooking.this, BookTableActivity.class);
                            startActivity(intent);
                        }
                    }, 4000);
                } else {
                    Toast.makeText(FamilyRoomBooking.this, "Please Try Again", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    public void reflectBooking(int bookedTable){
        ...
        switch (bookedTable){
            ...
            default:
                Toast.makeText(FamilyRoomBooking.this, "An Unexpected Error Occurred", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void showBookingDetails(final int tableNumber){
        ...
        View dialogView = LayoutInflater.from(FamilyRoomBooking.this).inflate(R.layout.bookedtableconfirmation, viewGroup, false);
        AlertDialog.Builder builder = new AlertDialog.Builder(FamilyRoomBooking.this);
        ...
        ambiance.setText("FAM JAM NIGHT , TAM's DINEOUT");
